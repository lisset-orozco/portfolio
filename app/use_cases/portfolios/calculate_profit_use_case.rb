...
module Portfolios
  class CalculateProfitUseCase < ApplicationUseCase
    ...
    def call
      ...
      response(error: [StandardError.new(self), e])
    end
    ...
    def search_stock_histores
      @histories_by_stock = portfolio.portfolio_stocks.each_with_object({}) do |portfolio_stock, hash|
    ...
    def calculate_amount_and_price_by_stock
      @stock_details = histories_by_stock.each_with_object({}) do |(symbol, histories), hash| 
        hash[symbol] = {
          total_stock: histories.map(&:amount).sum,
          total_invested: histories.map(&:price).sum
        }
      end
    end
    ...
    def calculate_profit
      ...
      stock_details.each do |symbol, values|
        ...
        profit_by_stock[symbol] = '%.2f' % profit
        total_profit += profit
      end

      {
        profit_by_stock: profit_by_stock, 
        annualized_profit: '%.2f' % ((total_profit / days) * 365)
      }
    end
  end
end
